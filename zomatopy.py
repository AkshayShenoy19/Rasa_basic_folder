...
class Zomato:
    ...
    def modrestaurant_search(self, query="", latitude="", longitude="", cuisines="", start=0, limit=5):
        """
        Takes either query, latitude and longitude or cuisine as input.
        Returns a list of Restaurant IDs.
        """
        ...
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(
            base_url + "search?q=" + str(query) + "&start=" + str(start) + "&count=" + str(limit) + "&lat=" + str(
                latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines), headers=headers).content).decode(
            "utf-8")
        urlSent = base_url + "search?q=" + str(query) + "&start=" + str(start) + "&count=" + str(limit) + "&lat=" + str(
